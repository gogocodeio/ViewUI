...
export default {
  ...
  methods: {
    close() {
      this.visible = false
      $emit(this, 'update:modelValue', false)
      $emit(this, 'on-cancel')
    },
    handleMask() {
    ...
    handleMoveMove(event) {
      ...
      if (this.sticky) {
        ...
        if (
          this.dragData.x + diff_distance.x <= this.stickyDistance &&
          diff_distance.x < 0
        ) {
          this.dragData.x = 0
        } else if (
          this.dragData.x + this.dragData.rect.width - clientWidth >
            -this.stickyDistance &&
          diff_distance.x > 0
        ) {
        ...
        if (
          this.dragData.y + diff_distance.y <= this.stickyDistance &&
          diff_distance.y < 0
        ) {
          this.dragData.y = 0
        } else if (
          this.dragData.y + this.dragData.rect.height - clientHeight >
            -this.stickyDistance &&
          diff_distance.y > 0
        ) {
