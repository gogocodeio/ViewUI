...
export default {
  name: 'iSelectHead',
  mixins: [mixinsChildren, Emitter, Locale],
  components: { Icon },
  props: {
    disabled: {
      type: Boolean,
  ...
  computed: {
    ...
    arrowType() {
      ...
      if (window.$IVIEW) {
        if (window.$IVIEW.select.customArrow) {
          type = ''
        } else if (window.$IVIEW.select.arrow) {
          type = window.$IVIEW.select.arrow
        }
      }
      return type
    },
    ...
    customArrowType() {
      ...
      if (window.$IVIEW) {
        if (window.$IVIEW.select.customArrow) {
          type = window.$IVIEW.select.customArrow
        }
      }
      return type
    },
    ...
    arrowSize() {
      ...
      if (window.$IVIEW) {
        if (window.$IVIEW.select.arrowSize) {
          size = window.$IVIEW.select.arrowSize
        }
      }
      return size
    },
